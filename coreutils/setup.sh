...
[[ -n ${coreutils_src} ]] || >&2 echo "setup: expected non-empty \$coreutils_src"
...
function configure_phase() {
    ...
    # --build                    generate build configuration
    # --enable-install-program   enable building the 'hostname' program
    #
...
function install_phase() {
	...
	# move /usr/bin/chroot to /usr/sbin/chroot, since
	# some programs hardcode /usr/sbin/chroot
	#
	mv -v ${LFS}/usr/bin/chroot ${LFS}/usr/sbin/
	# move chroot manpage to section 8
