...
function configure_phase() {
    ...
    # --prefix           create executabless that expect to run from /usr tree
    # --host             target architecture
    # --build            generate build configuration
    # --disable-static   don't generate static libraries
    # --docdir           fix documentation location
    #
    ./configure --prefix=/usr \
		--host=${LFS_TGT} \
		--build=$(build-aux/config.guess) \
		--disable-static \
		--docdir=/usr/share/doc/${vsubdir}
